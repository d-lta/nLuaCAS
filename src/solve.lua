-- solve.lua - Polynomial equation solver
-- Supports linear through quartic equations using standard algebraic methods

local ast = rawget(_G, "ast") or require("ast")
local errors = _G.errors

-- Helper to format expressions nicely
...
local function analyzeComplexity(equation_str, coeffs, degree)
    ...
    elseif degree == 2 then
        complexity.score = 2
        -- Check if it's a simple form like ax^2 = b
...
local function matchQuadraticEq(eq, var, steps)
    ...
    -- Check if this is a trivial quadratic (ax^2 + c = 0)
